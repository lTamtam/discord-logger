...
export function generateMasterKey(size?: number): string {
    if (!size) size = 256;
    const iVector = crypto.randomBytes(size);
...
export function encrypt(text: string | null, masterkey?: string): string {
    ...
    const key = crypto.pbkdf2Sync(masterkey, salt, ITERS, 32, 'sha512');
    const cipher = crypto.createCipheriv(ALGO, key, iVector);
    const encypted = Buffer.concat([cipher.update(text, 'utf-8'), cipher.final()]);
    const tag = cipher.getAuthTag();
    return Buffer.concat([salt, iVector, tag, encypted]).toString('base64');
};
...
export function decrypt(text: string, masterkey?: string): string {
    ...
    const encypted = data.subarray(96).toString('base64');
    const key = crypto.pbkdf2Sync(masterkey, salt, ITERS, 32, 'sha512');
    const decipher = crypto.createDecipheriv(ALGO, key, iVector);
    decipher.setAuthTag(tag);
    return decipher.update(encypted, 'base64', 'utf8') + decipher.final('utf8');
};