...
import { MESSAGES_EXPIRATION } from '../constants';
...
export async function getDbMessage(messageId: Snowflake): Promise<CacheMessageObject | null> {
    try {
        ...
        if (dbMessage) return { ...dbMessage, content: dbMessage.content ? decrypt(dbMessage.content) : '<No content>', createdAt: new Date(dbMessage.createdAt) };
    }
    catch (err) {
        logger.error({
            app: 'Database',
    ...
    try {
        await prisma.message.update({
            ...
            data: {
                ...
                content: encrypt(message.content ?? '<No content>'),
                guild: {
                    connectOrCreate: {
                        create: {
                            id: message.guild.id
