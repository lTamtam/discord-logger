...
import { getMember, getUser } from '../../utils/helpers';
...
const event: BotEvent = {
    ...
    execute: async (rule: AutoModerationRule) => {
        ...
        const automoderationRuleCreateEvent: WebhookEvent = {
            ...
            guild: rule.guild,
            eventName: eventName,
            eventBits: EventsBits.AutoModerationRuleCreate,
            embeds: [{
                author: {
                    name: `${user?.tag ?? 'Unknown user'} ${member && member.nickname ? `(${member.nickname})` : ''}`,
                    iconURL: user?.avatarURL() ?? Bun.env.USER_DEFAULT_AVATAR!
                },
                description: `${AUTOMOD_TRIGGER_TYPE_MAP[rule.triggerType]} automod rule was created`,
                fields: [
